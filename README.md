...
The employee must be authenticated before applying for leave.
A valid leave type must be selected.
The start date cannot be after the end date.
The employee should have sufficient leave balance.
Only authorized managers can approve or reject requests.
Only pending requests can normally be approved or rejected.
Approval and rejection actions should be recorded in the approval history.
Rejected requests should not reduce the employee's leave balance.
Invalid or non-existing leave requests should not be processed.
